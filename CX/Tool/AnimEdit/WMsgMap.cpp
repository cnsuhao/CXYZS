...
WMsgMapItem gWMsgData[] =
{
    { 0xFFFFFFFF ,"WM_SETTINGCHANGE"},
...
std::map<int, const char*> gWMsgMap;

const char* TranslateWMsg(int id){    if (gWMsgMap.empty())
    {
        int size = sizeof(gWMsgData)/sizeof(gWMsgData[0]);
        for (int i = 0; i < size; ++i)
        {
            gWMsgMap.insert(std::pair<int,const char*>(gWMsgData[i].id,gWMsgData[i].msgStr));
        }
    }    std::map<int, const char*>::iterator it = gWMsgMap.find(id);    if (it != gWMsgMap.end())
    {
        return it->second;
    }    return NULL;}
//if (TranslateWMsg(uMsg))
//{
//    char buff[32];
